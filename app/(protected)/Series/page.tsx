...
export default function Page() {
    ...
    return (
        <div>
            ...
            <div className='p-6'>
                <ClubTable data={series} key={JSON.stringify(series)} deleteSeries={deleteSeries} editSeries={null} viewSeries={(seriesId: string) => viewSeries(seriesId)} />
            </div>
            {userHasPermission(user, AVAILABLE_PERMISSIONS.editSeries) ?

                <div className="p-6">
                    <p id='seriesAddRace' onClick={createModal.onOpen} className="cursor-pointer text-white bg-blue-600 hover:bg-pink-500 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-3 md:mr-0">
                        Create New Series
                    </p>
                </div>
                : <></>

...
export default function ActivityMap({ raceId, windowHeight, activityId }: { raceId: string; windowHeight: number; activityId: string }) {
    ...
    const [activityData, setActivityData] = useState<Types.Activity>({} as Types.Activity)
    ...
    async function fetchActivityPositions(activity: Types.Activity): Promise<Types.Activity> {
        console.log(activity)
        let positions = await getActivityPositionsMutation.mutateAsync(
            {
                activityId: activityId
            },
            {
                onSuccess(positions) {
                    console.log(`Fetched positions for ${activity.id}:`, positions)
                },
                onError(error) {
                    console.log('Failed to fetch positions for ' + activity.id, error)
                }
            }
        )
        return { ...activity, position: positions }
    }
    ...
    useEffect(() => {
        ...
        if (activity) {
            const fetchAll = async () => {
                try {
                    const results = await fetchActivityPositions(activity)
                    console.log('Fetched activity positions', results)
                    if (mounted) setActivityData(results)
                } catch (err) {
                    console.error('Error fetching activity positions', err)
                }
            }
    ...
    useEffect(() => {
        ...
        if (isPlaying && activityData.position) {
            const positions = activityData.position.map((pos: any) => [pos.lat, pos.lon] as LatLngExpression)
        ...
        <div ref={wrapperRef} style={{ position: 'relative', height: windowHeight, width: '100%', zIndex: 2 }}>
            ...
            <MapContainer center={[51.6, -1.9]} zoom={ZOOM_LEVEL} style={{ height: '80%', width: '100%' }}>
                ...
                {activityData.position &&
                    (() => {
                        const points: any[] = activityData.position.map((pos: any) => [pos.lat, pos.lon]) || []
                        return <Polyline key={activityData.id} positions={points} pathOptions={{ color: '#FF00FF', weight: 4 }} />
                    })()}
                {currentPosition && (
                    <Circle
            ...
            <div className='controls-container' style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', padding: '10px' }}>
                ...
                <input
                    type='range'
                    min='0'
                    max={activityData.position ? activityData.position.length - 1 : 0}
                    value={scrubIndex}

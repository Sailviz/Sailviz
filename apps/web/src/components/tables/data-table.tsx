...
import { type ColumnDef, flexRender, getCoreRowModel, getPaginationRowModel, type PaginationState, useReactTable } from '@tanstack/react-table'
...
export function DataTable<TData, TValue>({
    ...
    const [currentPage, setCurrentPage] = useSearchParam<number>(pageName, 1, { parse: parseNumberSearchParam })
    const [pageSize, setPageSize] = useSearchParam<number>(limitName, pageSizeOptions[0] ?? 10, { parse: parseNumberSearchParam })

    const paginationState: PaginationState = {
        pageIndex: Math.max(currentPage - 1, 0), // zero-based index for React Table
        pageSize: pageSize
    }
    if (!data) {
        data = []
    ...
    const handlePaginationChange = (updaterOrValue: PaginationState | ((old: PaginationState) => PaginationState)) => {
        ...
        void setCurrentPage(pagination.pageIndex + 1) // converting zero-based index to one-based
    ...
    return (
        <div className='flex flex-1 flex-col space-y-4 min-h-96'>
                    ...
                    <div className='flex-1 text-sm text-muted-foreground'>
                        {totalItems > 0 ? (
                            <>
                                ...
                                {Math.min((paginationState.pageIndex + 1) * paginationState.pageSize, totalItems)} of {totalItems} entries
                            </>
                        ) : (
                            'No entries found'
                        )}

...
export default function PursuitCard({ result, raceState, lapBoat, showRetireModal }: { result: ResultsDataType, raceState: raceStateType, lapBoat: (id: string) => void, showRetireModal: (id: string) => void }) {
            ...
            <div>
                <div id={result.id} className={'bg-green-300 border-2 border-pink-500'}>
                    <div className="flex flex-row m-4 justify-between">
                        <h2 className="text-2xl text-gray-700 flex my-auto mr-5">{result.SailNumber} - {result.boat?.name} : {result.Helm} - {result.Crew} -</h2>
                        <div className="flex">
                            <p onClick={(e) => { confirm("are you sure you want to retire " + result.SailNumber) ? showRetireModal(result.id) : null; }} className="cursor-pointer text-white bg-blue-600 font-medium rounded-lg text-sm p-5 mx-2 ml-auto text-center flex">
                                Retire
                            </p>

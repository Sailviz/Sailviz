...
const envPath = monorepoRoot + `/packages/queue/.env.${env}`;
...
const lines = Object.entries(parsed).map(([key, value]) => {
  // If the value is numeric, output it as a number
  const numeric = Number(value);
  const isNumber = !isNaN(numeric);

  return `export const ${key} = ${isNumber ? numeric : JSON.stringify(value)};`;
});
...
fs.writeFileSync(monorepoRoot + "/packages/queue/src/config.ts", output);
